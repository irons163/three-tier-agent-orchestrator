...
V2 若支援 `task_name`，名稱會使用可辨識的工作前綴與固定角色結尾，例如 `security_review_sol_max` 或 `run_tests_luna_max`。Skill 會在啟動前確認名稱結尾與實際 model／reasoning 一致；V1 不傳入此欄位。
...
When V2 supports `task_name`, the name uses a recognizable task prefix and a fixed role suffix, such as `security_review_sol_max` or `run_tests_luna_max`. Before launch, the skill verifies that the suffix matches the actual model and reasoning arguments. V1 does not receive this field.
